...
| Scoring | per-seat score = cumulative `log.score` (`min(comb_lvl, prof_lvl)` summed over that seat's ended lives) + final life's `min_comb_prof`; results ranked by score; league ranking **Elo** over per-episode relative scores | matches upstream's own eval harness ranking; goal directive says Elo standings |
